...
export class DealsService {
    ...
    getLostReasons(): Observable<GenericResponse> {
        const getUrl = `${environment.apiUrl}/administration-company-setting-lost-reason`;
    ...
    updateDeal(deal: any): Observable<GenericResponse> {
        const updateUrl = `${environment.apiUrl}/deals/update-all`;
        const headers = new HttpHeaders({
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${this.layoutService.getLoggedUser()?.token}`,
        ...
        return this.http.put<GenericResponse>(updateUrl, JSON.stringify([deal]), { headers });
